...
- The autocomplete popup layout is localized cleanly: `src/ui/AutocompleteList.tsx` owns the suggestion surface, while `PromptShell` only needs to budget one extra row when that surface reserves a non-scrolling spacer row above the suggestions.
- The visual issue was better solved with layout than chrome: a blank parent row above the suggestion list plus row-owned horizontal padding gives the prompt separation the user wanted without adding borders or prompt-junction characters.
...
- Updated `src/ui/AutocompleteList.tsx` so suggestions render in a plain parent container with one non-scrolling spacer row above the scrollbox instead of a framed popup.
- Moved autocomplete horizontal padding onto each suggestion row so the selected background fills the visible blank columns on either side of the suggestion text.
- Kept `PromptShell` height accounting at one extra row only when autocomplete is visible, but removed the temporary prompt-shell `├`/`┤` border-character experiment.
- Reworked `test/autocompleteRender.test.ts` and its helper to assert the blank top row plus full-width selected padding coverage, and removed the temporary prompt border render helper/spec.
...
- Status extraction validation passed:
	...
	- `bunx tsc --noEmit`
- Autocomplete spacer-row validation passed:
	- `bun test test/autocompleteRender.test.ts`
...
- Placeholder revert validation passed:
	- `bun test test/config.test.ts test/autocomplete.test.ts test/autocompleteRender.test.ts test/completions.test.ts test/historyStore.test.ts`
	- `bunx tsc --noEmit`
- Focused autocomplete regression validation passed:
	- `bun test test/autocomplete.test.ts test/autocompleteRender.test.ts test/completions.test.ts test/historyStore.test.ts`
